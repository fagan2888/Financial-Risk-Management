...
def Form9p41(lt,t,alphl,alphs,sl,ss,rh):
    #Compute formula 9.41
    ...
    biga=(alphl-alphs+(ss**2)/2-rh*sl*ss)/10000
    d2=d1+(biga-bigsigma_sq/2)*np.sqrt(t)/np.sqrt(bigsigma_sq)
    more_pain=np.exp(-lt*(1-2*biga/bigsigma_sq))*norm.cdf(d2)
...
Z=100.0*Form9p41(logterm,time,alpha_l,alpha_s,Y,X,rho)
